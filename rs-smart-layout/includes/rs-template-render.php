...
function rs_render_child_terms( $child_terms, $taxonomy ) {
	...
	echo '<div class="rs-card-grid rs-child-terms">';
	...
	foreach ( $child_terms as $term ) {
		...
		<a href="<?php echo esc_url( $term_link ); ?>" class="<?php echo esc_attr( $card_class ); ?>">
			...
			<?php endif; ?>
			<h3 class="rs-card-title"><?php echo esc_html( $term->name ); ?></h3>
		</a>
		<?php
	}
...
function rs_render_posts( $query ) {
	if ( ! $query || ! $query->have_posts() ) {
		echo '<p class="rs-no-results">' . esc_html__( '目前沒有內容。', 'rs-smart-layout' ) . '</p>';
		return;
	}

	echo '<div class="rs-card-grid rs-posts">';
	...
	while ( $query->have_posts() ) {
		...
		<a href="<?php echo esc_url( $post_link ); ?>" class="<?php echo esc_attr( $card_class ); ?>">
			...
			<?php endif; ?>
			<h3 class="rs-card-title"><?php echo esc_html( $post_title ); ?></h3>
		</a>
		<?php
	}
...
function rs_render_pagination( $query ) {
	...
	$pagination = paginate_links(
		array(
			...
			'prev_text' => '&laquo; ' . __( '上一頁', 'rs-smart-layout' ),
			'next_text' => __( '下一頁', 'rs-smart-layout' ) . ' &raquo;',
			'type'      => 'list',
		)
	);
